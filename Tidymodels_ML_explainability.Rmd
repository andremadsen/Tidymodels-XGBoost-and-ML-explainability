...
# Classification performance by ROC AUC
```{r}
roc_test <- best_model %>%
  collect_predictions() %>%
...
# Finalize and save the final XGBoost model object
```{r}
...
# Load the ML object in other sessions to assign prediction scores to classify healthy/disease
```{r}
object    <- base::load("final_xgb.RData")
final_xgb <- base::get(object)
```
